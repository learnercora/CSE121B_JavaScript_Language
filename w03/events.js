...
function renderTasks(tasks) {
  ...
  // loop through the tasks array. for each of 
  //them we need to add the HTML markup for a todo.
...
function newTask() {
  ...
  // render out the list.
    ...
    tasks.push({ detail: newTask, completed: false});
    renderTasks(tasks);
}
...
function completeTask(taskElement) {
  ...
  // If the first part is true (left of the ?), then the value on the left 
  //of the : will get returned, otherwise the value on the right of the : will be returned.
...
//   console.log(event.currentTarget);
  ...
  //parent li to work with however. HINT: Remember element.closest()? Look it up if you don't
  ...
  //console.log(event.target.dataset.function);
...
const buttonSubmitTask = document.getElementById('submitTask');
buttonSubmitTask.addEventListener("click", newTask);
